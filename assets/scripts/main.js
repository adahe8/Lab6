...
function addRecipesToDocument(recipes) {
  ...
  let recipeParent = document.querySelector("main");
  ...
  recipes.forEach((recipe) => {
    ...
    recipeParent.appendChild(recipeCard);
  });
}
...
function initFormHandler() {
  ...
  const recipeInfo = document.querySelector("form");
  // B3. TODO - Add an event listener for the 'submit' event, which fires when the
  //            submit button is clicked
  recipeInfo.addEventListener("submit", function (e) {
    e.preventDefault();
    const formData = new FormData(recipeInfo);
    let recipeObject = {};
  ...
  const clearStorage = document.querySelector(".danger");
